package com.twilio.voice.sufalamtech.fcm;
...
import com.twilio.voice.sufalamtech.base.Constants;
import com.twilio.voice.sufalamtech.IncomingCallNotificationService;
...
public class VoiceFirebaseMessagingService extends FirebaseMessagingService {
    ...
    public void onMessageReceived(RemoteMessage remoteMessage) {
        ...
        if (remoteMessage.getData().size() > 0) {
            boolean valid = Voice.handleMessage(MyApplication.getInstance(), remoteMessage.getData(), new MessageListener() {
                @Override
                public void onCallInvite(@NonNull CallInvite callInvite) {
                    final int notificationId = (int) System.currentTimeMillis();
                    Log.d("CallerInfo", "From : " + callInvite.getCustomParameters().get("From") + " To : " + callInvite.getTo());
